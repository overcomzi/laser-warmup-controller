...
public class WarmupController {
    ...
    public void markLaserOn() {
        ...
        warmupStartTime = time.currentTimeMillis() / 1000.0;
    ...
    public void markLaserOff() {
        on = false;
        laserStopTime = time.currentTimeMillis() / 1000.0;
    }
    ...
    public double getRemainingTime() {
        ...
        return Math.max(0, currentWarmupDuration -
                (time.currentTimeMillis() / 1000.0 - warmupStartTime));
    }

    public WarmupController(TimeProvider time) {
        this.time = time;
    ...
    public WarmupController() {
        this.time = new SystemTimeProvider();
        warmupStartTime = laserStopTime = -thresholdDuration * 2;
        currentWarmupDuration = 0;
        on = false;
    }
